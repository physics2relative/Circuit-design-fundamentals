...
Naive AND gating과 latch-based ICG의 차이는 아래 프로젝트에서 waveform으로 확인할 수 있다.

```text
digital/projects/clock_gating_icg_experiment/
```

이 실습은 enable이 clock high 구간에서 변할 때 naive gating에는 mid-cycle gated clock edge가 생길 수 있고, latch-based ICG는 enable을 clock low phase에서만 반영해 이를 피하는 흐름을 보인다.

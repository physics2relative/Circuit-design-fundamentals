...
참고한 방향성: Hardware Interview의 Digital Design/RTL study guide에서 강조하는 Verilog, FSM, FIFO, arbiter, STA, protocol/interface, CDC, RTL coding challenge 범위를 바탕으로 재구성했습니다.
...
- `wire`: 회로의 연결선을 표현하는 net 타입입니다. `assign` 문이나 module port 연결에 주로 사용합니다.
- `reg`: `always`나 `initial` 같은 procedural block 안에서 값을 대입받는 변수 타입입니다. 이름과 달리 항상 실제 register를 의미하지는 않습니다.

면접 답변:

> `wire`는 continuous assignment로 구동되는 연결선이고, `reg`는 procedural assignment 대상입니다. `reg`가 clocked always block에서 쓰이면 flip-flop으로 합성될 수 있고, 조합 always block에서 쓰이면 조합논리로 합성될 수 있습니다.

### Blocking `=` vs Non-blocking `<=`

- Blocking `=`: 현재 procedural block 안에서 문장 순서대로 즉시 값이 갱신되는 것처럼 동작합니다.
...
`always @(*)`는 조합논리 block의 sensitivity list를 자동으로 구성합니다. 직접 sensitivity list를 쓰다가 입력을 누락하면 simulation과 합성 결과가 달라질 수 있으므로, 조합논리에는 `always @(*)`를 쓰는 것이 안전합니다.
